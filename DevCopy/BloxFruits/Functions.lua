    ...
    SafeSpawn(function()
        ...
        while task.wait(0.08) do
            ...
            pcall(function()
                ...
                -- Combate estilo Tiro: CFrame contínuo em cima + bring no chão
                repeat
                    task.wait()
...
function Functions.StartAutoRaid(config)
	...
	-- Lógica:
	--   1. Assim que ativa: fica no ar procurando inimigos perto do player
	--   2. Em paralelo, monitora se uma ilha nova spawnou perto dele
	--   3. Se achar ilha próxima: voa até o centro dela e continua
	--      procurando/matando inimigos a partir de lá
	--   4. Mata mobs um a um (puxa pra baixo do player); ilha 5 prioriza boss
	-- ==========================================
	SafeSpawn(function()
		...
		local homeCF      = nil  -- posição de referência atual (centro da ilha ou onde começou)

		-- Busca a ilha que exista perto do player (varre 1 a 5)
		...
		-- Procura boss (ilha 5) ou mob normal num raio do ponto de referência
		local function FindTarget(enemies, refPos, bossOnly)
			for _, v in ipairs(enemies:GetChildren()) do
				...
				if (vHrp.Position - refPos).Magnitude <= 5000 then
					return v
				end
			end
			return nil
		...
		while task.wait(0.1) do
			...
			pcall(function()
				...
				-- Define ponto de referência inicial (onde começou a raid) se ainda não tem
				if not homeCF then
					homeCF = hrp.CFrame
				end

				-- ---- MONITORAMENTO DE ILHA (roda sempre, em paralelo ao combate) ----
				local island, centerPos, islandNum = FindNearestIsland()

				if island and island ~= lastIsland then
					-- Ilha nova detectada perto: define como novo ponto de referência e voa
					print("[AutoRaid] Ilha " .. tostring(islandNum) .. " spawnou perto! Indo até ela...")
					lastIsland = island
					homeCF     = CFrame.new(centerPos.X, centerPos.Y + 55, centerPos.Z)
					atCenter   = false
					isFlying   = false
				end

				-- ---- VOO ATÉ O PONTO DE REFERÊNCIA ATUAL ----
				local distHome = (hrp.Position - homeCF.Position).Magnitude
				...
				elseif distHome > 80 then
					-- Se afastou muito do ponto de referência: volta
					FlyAsync(homeCF)
					return
				end

				-- Garante PartTele ativo (player sempre no ar, nunca cai)
				if not workspace:FindFirstChild("RaidPartTele") then
					Functions.FlyToRaid(homeCF, config)
				end

				-- ---- BUSCA E MATA INIMIGOS PERTO ----
				local enemies = workspace:FindFirstChild("Enemies")
				if not enemies or _killing then return end

				-- Se estamos na ilha 5: prioridade total pro boss
				if islandNum == 5 then
					local boss = FindTarget(enemies, homeCF.Position, true)
					if boss then
						print("[AutoRaid] Boss detectado (ilha 5): " .. boss.Name)
						_killing = true
						SafeSpawn(function()
							KillBossInstant(boss, homeCF)
							atCenter = true
							_killing = false
						end)
						return
				...
				-- Mobs normais (procurando perto do ponto de referência atual)
				local mob = FindTarget(enemies, homeCF.Position, false)
				if mob then
					print("[AutoRaid] Mob detectado: " .. mob.Name)
					_killing = true
					SafeSpawn(function()
						KillMob(mob, homeCF)
						atCenter = true
				...
				-- Sem mobs/boss perto: fica no ar aguardando spawn ou ilha nova
			end)
		end
	end)
end
...
-- =====================================================
-- HOVER SYSTEM - mantem o player SUSPENSO NO AR (nunca
-- encosta no chao) enquanto qualquer funcao de auto-farm
-- estiver ativa. Usa o mesmo mecanismo ja comprovado da
-- raid (PartTele + Heartbeat + Humanoid em estado Physics),
-- so que de forma continua/persistente em vez de um voo
-- unico. Resolve a queda pela raiz: se o player nunca toca
-- o chao durante o farm, nao tem como cair nele.
-- =====================================================
local _hoverPart           = nil
local _hoverConn           = nil
local _hoverPrevStates     = nil
local _hoverPrevAutoRotate = nil
local _hoverActive         = false

function Functions.IsHovering()
    return _hoverActive
end

function Functions.StartHover(config)
    if _hoverActive then return end
    local char = Player.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    local hum  = char and char:FindFirstChildOfClass("Humanoid")
    if not char or not hrp or not hum or hum.Health <= 0 then return end
    ...
    local pt        = Instance.new("Part")
    pt.Size         = Vector3.new(4, 1, 4)
    pt.Name         = "HoverPartTele"
    pt.Anchored     = true
    pt.Transparency = 1
    pt.CanCollide   = false
    pt.CFrame       = hrp.CFrame
    ...
    _hoverPrevStates     = {}
    _hoverPrevAutoRotate = hum.AutoRotate
    for _, st in ipairs({
        Enum.HumanoidStateType.Freefall,
        Enum.HumanoidStateType.Jumping,
        Enum.HumanoidStateType.Landed,
        ...
        _hoverPrevStates[st] = hum:GetStateEnabled(st)
        hum:SetStateEnabled(st, false)
    end
    hum.AutoRotate = false
    hum:ChangeState(Enum.HumanoidStateType.Physics)
    ...
    -- Sobe suavemente ate a altura de hover
    local riseTarget = Vector3.new(hrp.Position.X, targetY, hrp.Position.Z)
    local riseTween = TweenService:Create(pt, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { CFrame = CFrame.new(riseTarget) })
    riseTween:Play()

    _hoverConn = RunService.Heartbeat:Connect(function()
        local c    = Player.Character
        local cHrp = c and c:FindFirstChild("HumanoidRootPart")
        if cHrp and _hoverPart and _hoverPart.Parent then
            local _, yaw, _ = cHrp.CFrame:ToOrientation()
            cHrp.CFrame = CFrame.new(_hoverPart.Position) * CFrame.Angles(0, yaw, 0)
        end
    end)
end

-- Move o ponto de hover horizontalmente (mantendo a altura atual) ate
...
-- criterio generico do MovementBlocker (qualquer funcao ativa que nao
-- esteja na lista de excecao = player deve estar flutuando).
-- FIX: o Hover so pode ligar para funcoes que foram REESCRITAS pra
-- usar esse sistema. Se ligasse para QUALQUER funcao ativa (ex:
-- AutoFarmLevel, AutoFarmNearest, Raid), o _hoverConn (Heartbeat que
-- trava o player no PartTele do hover) ficaria escrevendo no
-- HumanoidRootPart.CFrame AO MESMO TEMPO que o Heartbeat interno do
-- FlyToPosition (usado por essas outras funcoes, que ainda voam ate
-- o chao perto do mob) - dois sistemas brigando pelo mesmo CFrame a
-- cada frame causa tremedeira/teleporte. Por enquanto so o
    ...
    SafeSpawn(function()
        while task.wait(0.15) do
            ...
            if not hum or hum.Health <= 0 then
                if _hoverActive then Functions.StopHover() end
                continue
            end

            local shouldHover = false
            for key in pairs(HOVER_TRIGGER_KEYS) do
            ...
            if shouldHover and not _hoverActive then
                Functions.StartHover(config)
            elseif not shouldHover and _hoverActive then
                Functions.StopHover()
            end
        end
    end)
...
-- =====================================================
-- TIRO-STYLE: anti-queda + posição acima do mob
-- =====================================================
function Functions.EnableFarmClip()
    local char = Player.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
...
function Functions.DisableFarmClip()
    local char = Player.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
...
-- Fica em cima do mob (força CFrame agora)
function Functions.StayAboveMob(mob, flyOffset)
    local char = Player.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    local mhrp = mob and mob:FindFirstChild("HumanoidRootPart")
    if not hrp or not mhrp then return end
    local offset = tonumber(flyOffset) or 15
    local _, yaw = hrp.CFrame:ToOrientation()
    hrp.CFrame = CFrame.new(mhrp.Position + Vector3.new(0, offset, 0)) * CFrame.Angles(0, yaw, 0)
end

-- Bring no CHÃO do mob (não enterra)
function Functions.BringMobToGround(mobName, refMob, maxDist)
    local char = Player.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    local refHrp = refMob and refMob:FindFirstChild("HumanoidRootPart")
    if not hrp or not refHrp then return end
    local distMax = tonumber(maxDist) or 350
    local groundY = refHrp.Position.Y
    ...
    for _, om in ipairs(enm:GetChildren()) do
        if om.Name == mobName then
            ...
            if ohrp and ohum and ohum.Health > 0 then
                ...
                if d <= distMax then
                    Functions.LockMobInPlace(om, CFrame.new(hrp.Position.X, groundY, hrp.Position.Z))
                end
            end
        end
    end
